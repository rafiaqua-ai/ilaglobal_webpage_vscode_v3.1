...
import { getGlobalCourses, getGlobalPaths, GlobalCourse, GlobalPath } from '../lib/db';
...
export default function EducationPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('ilas-paths-changed', loadCourses);
    return () => {
      window.removeEventListener('ilas-courses-changed', loadCourses);
      window.removeEventListener('ilas-paths-changed', loadCourses);
    };
  }, [activeNav]);
          ...
          <section id={activeCourse.id} className="scroll-mt-32 pt-10">
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-start mb-8">
              ...
              <div className="lg:col-span-6 flex flex-col gap-2">
                {(() => {
                  const matchingPaths = paths.filter(p => p.linkedCourseId === activeCourse.id || p.id === activeCourse.pathId);
                  const displayPathItems = matchingPaths.length > 0 ? matchingPaths : (paths.length > 0 ? paths : [{ id: 'default', name: activeCourse.methods || 'Standard Pathway', methods: 'AI + Adaptive', starting: '', ending: '', remarks: '' }]);
                  
                  return (
                    <>
                      <div className="flex items-center justify-between px-1">
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                          Available Delivery Paths ({displayPathItems.length})
                        </span>
                        {displayPathItems.length > 3 && (
                          <span className="text-[10px] text-brand-600 font-bold bg-brand-50 border border-brand-200 px-2 py-0.5 rounded-full animate-pulse">
                            ↕️ Scroll to view all ({displayPathItems.length})
                          </span>
                        )}
                      </div>
                      ...
                      <div className="space-y-3 max-h-[380px] overflow-y-auto pr-1.5 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 rounded-2xl">
                        {displayPathItems.map((p, pIdx) => {
                          const isActive = pIdx === 0 || p.id === activeCourse.pathId;
                          const pathTitle = p.name || activeCourse.methods;
                          const pathMethod = p.methods || 'Standard Adaptive Plan';
                          return (
                            <div 
                              key={p.id || pIdx} 
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                                <button onClick={() => scrollTo(`${activeCourse.id}-details`)} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center gap-1">
                                  <BookOpen className="w-3.5 h-3.5" /> Details
                                </button>
                                <button onClick={openTrial} className="px-3 py-1.5 bg-brand-50 hover:bg-brand-100 text-brand-700 text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center gap-1">
                                  <Play className="w-3.5 h-3.5" /> Free Trial
                                </button>
                                <button onClick={() => navigateTo(`#applications?course=${encodeURIComponent(activeCourse.name)}&path=${encodeURIComponent(pathTitle)}`)} className="px-4 py-1.5 bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold rounded-xl cursor-pointer transition-all shadow-xs">
                                  Enroll
                                </button>
                              </div>
                            </div>
